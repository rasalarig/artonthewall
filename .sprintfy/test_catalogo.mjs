...
// CHECK 5: Test price range filter
const priceBtn = await page.$('button:has-text("Ate R$500")') || await page.$('button:has-text("Até R$500")');
if (priceBtn) {
...
const allPriceBtns = await page.$$('button');
let highPriceBtn = null;
for (const btn of allPriceBtns) {
...
const allBtns2 = await desktopPage.$$('button');
let highBtn2 = null;
